...
def descarca_json_gz_din_drive(service, file_id):
    try:
        ...
        print(f"⚠️ Eroare citire MasterIndex.json.gz (ID {file_id}): {e}", flush=True)
        return None
        ...
        try:
            ...
            if arhive_gasite:
                ...
                for f_tar in arhive_gasite:
                    ...
                    # Extrage anul din nume: brut_XML_1990.tar.gz -> 1990
...
def unifica_in_master_index(reset_complet=False):
    ...
    print("============================================================", flush=True)
    print(f"🔄 UNIFICARE MASTER INDEX | Discul 0: {FOLDER_INDEX_ID}", flush=True)
    ...
    if files_master and not reset_complet:
        ...
        master_file_id = files_master[0]["id"]
        print("⚠️ [FULL RESET] Resetare completă! MasterIndex va fi golit și refăcut de la rădăcină.", flush=True)

    # 2. DACA E FULL RESET: Parcurgem mai întâi arhivele .tar.gz de pe discuri
    if reset_complet:
    ...
    print("\n============================================================", flush=True)
    print(f"✅ UNIFICARE MASTER INDEX FINALIZATĂ SUCCES!", flush=True)
