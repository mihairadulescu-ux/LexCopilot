...
def descarca_monitoare():
    ...
    with httpx.Client(timeout=15.0, follow_redirects=True) as client:
        for an in YEARS_TO_PROCESS:
            ...
            # Găsim ultimul valid înregistrat în CSV (20 sau 15)
            ultimul_valid = 0
            ...
            # Limita maximă: ultimul valid + 10 (pentru anul curent), sau ultimul valid (pentru anii trecuți)
            if an == AN_CURENT:
                limita_maxima_interogare = min(1500, max(1, ultimul_valid) + 10)
            else:
                limita_maxima_interogare = ultimul_valid
                
            print(f"📊 Plajă scanare stabilită: 1 ➔ {limita_maxima_interogare}")
