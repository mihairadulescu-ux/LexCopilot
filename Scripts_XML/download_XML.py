...
def proceseaza_descarcare_an(an, pagina_start=1):
    ...
    while True:
        # AICI ÎNLOCUIEȘTI CU TEMPLATE-UL TĂU EXACT DE URL:
        url_pagina = f"https://legislatie.just.ro/..." # <-- Inserare URL exact pentru an și pagină
        ...
        for incercare in range(1, MAX_RETRIES_PER_PAGE + 1):
            ...
            if ok:
                ...
                # =========================================================
                # AICI PUI LOGICA TA DE SALVARE XML PE DISC SAU DRIVE
                # =========================================================
                
                break  # Ieșim din bucla de retry
            else:
                ultimul_motiv_esec = motiv
                print(f"   ⚠️ Incercarea {incercare}/{MAX_RETRIES_PER_PAGE} eșuată pe pagina {pagina_curenta}. Pauză {pauza}s...", flush=True)
                time.sleep(pauza)
        
        if succes:
            pagina_curenta += 1
            # Aici poți pune o condiție de ieșire din buclă dacă ai atins ultima pagină a anului
        else:
...
if __name__ == "__main__":
    try:
        ...
        # PRINDERE GLOBALĂ A ERORILOR: Oprește prăbușirea prin `/usr/bin/bash -e` din GitHub Actions
        ...
        print("\n🛡️ Ieșim curat (exit 0) pentru a păstra toate log-urile în consola CI/CD.", flush=True)
        sys.exit(0)
