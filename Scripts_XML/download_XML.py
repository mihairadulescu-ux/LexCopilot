...
def get_already_downloaded_pages(service, target_year):
    """
    Scanează Drive și returnează un dicționar cu paginile deja descărcate.
    Sărim peste fișiere doar dacă dimensiunea lor este >= 20 bytes.
    """
    valid_pages = set()
    page_token = None
    ...
    try:
        while True:
            ...
            for file in response.get("files", []):
                ...
                if size < 20:
                    print(
                        f"{GALBEN}   ⚠️ Re-descărcare forțată: {name} are doar {size} bytes.{RESET}"
                    )
                    continue
...
def upload_to_drive(service, filename, content_bytes):
    ...
    try:
        ...
        print(
            f"{VERDE}✅ Fișier salvat în Drive: {filename} (ID: {file.get('id')}){RESET}"
        )
        return True
    except Exception as e:
        print(
            f"{ROSU}❌ Eroare upload Drive pentru {filename}: {e}{RESET}"
        )
        return False
...
def download_year(drive_service, composite_type_name, target_year):
    ...
    downloaded_pages = get_already_downloaded_pages(
        drive_service, target_year
    )
    ...
    if downloaded_pages:
        ...
        print(
            f"📦 {len(downloaded_pages)} pagini VALIDE în Drive pentru {target_year}. (Ultima scanată în siguranță: {max_page})"
        )
        ...
        if gaps:
            print(
                f"{GALBEN}🛠️ Detectat {len(gaps)} lacune/fișiere alterate în istoric: {gaps}. Începem repararea.{RESET}"
            )
            pages_to_process.extend(gaps)
        next_new_page = max_page + 1
    else:
        print("🆕 An complet nou în acest segment. Începem de la pagina 1.")
        next_new_page = 1
    ...
    while True:
        ...
        if (
            ...
            if not is_gap_repair:
                ...
                print(
                    "   ✍️ Generăm fișier XML martor minimal pentru a opri scanarea repetată."
                )
                xml_martor = b"<GrupLegi><Info>PaginaGoalaConfirmataJustRo</Info></GrupLegi>"
                success = upload_to_drive(
                    drive_service, filename, xml_martor
                )
                if success:
                    files_saved += 1
        else:
