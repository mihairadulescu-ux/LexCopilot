...
try:
    ...
    print("⚠️ Nu s-a putut importa 'XML_INDEX_READER.py'. Asigură-te că fișierul există în același director.")
    sys.exit(1)
...
def get_already_downloaded_pages(service, target_year):
    ...
    try:
        ...
        print(f"⚡ [Index Reader] Încărcate {len(valid_pages)} pagini existente pentru anul {target_year} din Indexul Virtual.")
        return valid_pages
    except Exception as e:
        print(f"{ROSU}⚠️ Eroare la încărcarea indexului virtual: {e}. Se începe scanarea curată.{RESET}")
        return set()


def upload_to_drive(service, filename, content_bytes):
    """Încarcă fișierul XML brut în Drive și returnează ID-ul generat."""
    try:
        ...
        file_id = file.get("id")
        print(f"{VERDE}✅ Fișier salvat în Drive: {filename} (ID: {file_id}){RESET}")
        return file_id
    except Exception as e:
...
def salveaza_micro_index_temp(service, flag_updates):
    """Creează și urcă un micro-index temporar în Drive cu fișierele adăugate recente."""
    if not flag_updates:
        return True
    ...
    try:
        json_bytes = json.dumps(continut_payload, ensure_ascii=False, indent=2).encode("utf-8")
        file_metadata = {"name": nume_micro_index, "parents": [FOLDER_TEMP_INDEXES_ID]}
        ...
        print(f"{ROSU}⚠️ Eroare la salvarea micro-indexului temporar: {e}{RESET}", flush=True)
        return False
...
def download_year(drive_service, composite_type_name, target_year):
    """Descarcă toate paginile lipsă pentru un singur an și salvează micro-indexul la fiecare 10 fișiere."""
    ...
    if downloaded_pages:
        ...
        print(f"📦 {len(downloaded_pages)} pagini VALIDE în index pentru {target_year}. (Ultima scanată: {max_page})")
    ...
    # Puffer local pentru micro-index (salvat din 10 în 10)
    mutații_micro_index = {}
    ...
    while True:
        ...
        if "<a:Legi>" not in raw_xml_string and "<Legi>" not in raw_xml_string:
            ...
            uploaded_id = upload_to_drive(drive_service, filename, raw_xml_bytes)
            if uploaded_id:
                files_saved += 1
                mutații_micro_index[filename] = {
                    "id": uploaded_id,
        ...
        # ---------------------------------------------------------------
        # FLUSH LA FIECARE 10 FIȘIERE DESCĂRCATE
        # ---------------------------------------------------------------
    ...
    # Flush final la ieșirea din buclă pentru fișierele rămase
