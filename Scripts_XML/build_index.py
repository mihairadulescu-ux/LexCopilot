...
# 1. Variabila pentru folderul de metadate
FOLDER_METADATA_ID = os.getenv("DRIVE_FOLDER_METADATA", "").replace('"', '').replace("'", "").strip()

# 2. Variabila pentru folderele de stocare XML
...
def descarca_index_existenta_din_drive(service):
    ...
    try:
        ...
        if fisiere:
            ...
            print(f"📥 [Cloud Sync] Încărcat indexul din Folderul de Metadate ({FOLDER_METADATA_ID[:8]}...).", flush=True)
...
def salveaza_index_in_drive(service):
    ...
    try:
        ...
        if fisiere:
            ...
            print(f"📤 [Cloud Sync] Indexul 'index_xml.json' a fost actualizat în Folderul de Metadate!", flush=True)
        else:
            metadata = {'name': CALE_INDEX_LOCAL, 'parents': [FOLDER_METADATA_ID]}
            service.files().create(body=metadata, media_body=media).execute()
            print(f"📤 [Cloud Sync] Indexul 'index_xml.json' a fost creat în Folderul de Metadate!", flush=True)
    except Exception as e:
        print(f"⚠️ Eroare la sincronizarea indexului cu Drive: {e}", flush=True)
...
def construieste_sau_actualizeaza_index():
        ...
        try:
            with open(CALE_INDEX_LOCAL, "r", encoding="utf-8") as f:
                ...
                if isinstance(data_stocata, dict) and "fisiere" in data_stocata:
                    ...
                    print(f"🧠 [Index Incremental] Încărcate {len(fisiere_map)} fișiere existente. Ultimul update: {last_updated}", flush=True)
        except Exception as e:
        ...
        try:
            while True:
                ...
                for f in files:
                    ...
                    # Păstrăm statusul Tags_extracted dacă exista deja în index, altfel îi dăm valoarea implicită False
                    ...
                    # UPDATE PERIODIC: Afișăm status la fiecare 1.000 de fișiere indexate
                    if contor_folder % 1000 == 0:
                        print(f"   ⚡ [Folder {folder_id[:8]}] Progres: {contor_folder} fișiere procesate... (Total global în index: {len(fisiere_map)})", flush=True)

                page_token = response.get('nextPageToken', None)
                if not page_token:
                    break
                    
            print(f"✅ [Folder Finalizat] Găsite/actualizate {contor_folder} fișiere în folderul {folder_id[:8]}.", flush=True)
    ...
    print(f"\n{VERDE}✅ [Index Salvat] Total în index: {len(fisiere_map)} fișiere. Actualizări noi în această rulare: {fisiere_noi_sau_modificate}. Ștampila: {acum_iso}{RESET}", flush=True)
